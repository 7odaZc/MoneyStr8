...
export default function TextInput({
  ...
  const inputId = id || props.name || `inp-${Math.random().toString(16).slice(2)}`;
  return (
    <div className={`w-full ${className}`}>
      ...
      <input
        id={inputId}
        className={`w-full rounded-lg bg-white/10 border px-3 py-2.5 text-sm text-white placeholder-white/40
